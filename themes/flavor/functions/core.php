...
function it_query_posts($query) {	
	...
	if(empty($query->query_vars['suppress_filters'])) {
		...
		#only do this for pages with loops that don't have the post explicitly set, like the home page		
		if((!$post_type || in_array($post_type, $allowed)) && !is_page() && !is_preview() && !is_attachment() && !is_search() && !is_admin()) {
...
function it_custom_login_logo() {
	if (it_get_setting("login_logo_url")) {
		$out = '<style type="text/css">#login h1 a { background:url('.it_get_setting("login_logo_url").') no-repeat center center !important; background-size: auto auto !important; }
    </style>';
    print $out;
	}
}
...
#google rich snippet structured data for reviews
	...
	$out .= '<div class="structured-data" itemscope itemtype="http://schema.org/Review">';
		...
		$out .= '<span itemprop="author">'.get_the_author().'</span>';
			...
			$out .= '<span itemprop="ratingValue">'.$total_rating.'</span>';
      		$out .= '<span itemprop="bestRating">'.$best_rating.'</span>';	
		$out .= '</div>';
	$out .= '</div>';
	echo $out;
}
