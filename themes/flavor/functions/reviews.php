...
function it_get_pros_cons($postid) {
	...
	#$positives = apply_filters('the_content',get_post_meta($postid, IT_META_POSITIVES, $single = true));	
	#$negatives = apply_filters('the_content',get_post_meta($postid, IT_META_NEGATIVES, $single = true));	
	$positives = get_post_meta($postid, IT_META_POSITIVES, $single = true);	
	$negatives = get_post_meta($postid, IT_META_NEGATIVES, $single = true);	
...
function it_get_bottom_line($postid) {
	...
	$disable_review = get_post_meta( $postid, IT_META_DISABLE_REVIEW, $single = true );
	if(empty($minisite) || $disable_review=='true') return false;
	#$bottomline = apply_filters('the_content',get_post_meta($postid, IT_META_BOTTOM_LINE, $single = true));
	$bottomline = get_post_meta($postid, IT_META_BOTTOM_LINE, $single = true);		
...
function it_show_rating($postid, $minisite, $single = false, $editor_rating_hide = false, $user_rating_hide = false) {
	...
	if($minisite->editor_rating_disable || $minisite->editor_rating_hide || $editor_rating_hide_override || empty($editor_rating)) {
		$editor_rating_hide=true;
		$solo_user=' solo';
	}
	if($minisite->user_rating_disable || $minisite->user_rating_hide || $user_rating_hide_override || empty($user_rating)) {
			...
			if(!$editor_rating_disable) {
				...
				$out.='">';
					$out.='<div class="ratings-panel editor-rating">';
						...
						$out.='<div class="rating-wrapper total">';
