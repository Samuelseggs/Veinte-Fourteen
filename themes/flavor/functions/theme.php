...
function it_header_scripts() { ?>
	...
	#default fonts   
    echo '<link href="http://fonts.googleapis.com/css?family=Oswald:300,400|Signika:400,300,600,700|Source+Sans+Pro:700" rel="stylesheet" type="text/css" />';
	?>
...
#get featured image
function it_featured_image($postid, $size, $width, $height, $wrapper = false) {
	$out = '';
	$featured_image = '';
	if($wrapper) $out.='<div class="featured-image-wrapper"><div class="featured-image-inner">';
	$featured_image .= get_the_post_thumbnail($postid, $size, array( 'title'=> '' ));
	if(empty($featured_image)) $featured_image .= '<img src="'.THEME_IMAGES.'/placeholder-'.$width.'.png" alt="featured image" width="'.$width.'" height="'.$height.'" />';
	$out.=$featured_image;
...
function it_get_likes($postid, $clickable = false, $label = false, $long_label = false) {
	...
	if($clickable) {
		$out.='<a class="like-button '.$postid.' info" data-postid="'.$postid.'" data-likeaction="'.$likeaction.'" title="'.__('Likes',IT_TEXTDOMAIN).'">';
	} else {
...
function it_get_comments($postid, $showifempty = false, $label = false) {
		...
		case 'categories':
			...
			foreach($terms as $term) {				
				...
				if(have_posts()) {
					...
					if($count==1) $filters .= ' active';
					$filters .= '" title="' . __('More articles in category: ', IT_TEXTDOMAIN) . $term->name . '"><span class="left-side"></span>'.$term->name.'<span class="right-side"></span></a>';				
