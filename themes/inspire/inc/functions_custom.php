...
FUNCTIONS
	...
	function mb_parse_comment_info_into_array($file) {
...
/****************************************************
mb_parse_comment_info_into_array
****************************************************/

	function mb_parse_comment_info_into_array($file) {
		$style_data = file_get_contents($file);
		$explode_array = explode("*/",$style_data);
		$explode_array = explode("/*",$explode_array[0]);
		$explode_array = explode("\n",$explode_array[1]);
		$info_array = array();
		foreach ($explode_array as $key => $value) {
			$temp_split_array = explode(": ", $value);
			if (isset($temp_split_array[0])) $temp_split_array[0] = trim($temp_split_array[0]);
			if (isset($temp_split_array[1])) $temp_split_array[1] = trim($temp_split_array[1]);
			if (isset($temp_split_array[1])) $info_array[$temp_split_array[0]] = $temp_split_array[1];
		}
