				...
				<!-- GENERAL -->
					...
					<div class='help'>
						...
						<span>
							...
							Turning off the responsive design will make the site look the same across all devices and browser sizes.
						</span>	
						<br><br>
						...
						<b>Automatically check for updates</b> <br>
						<span>
							Theme will automatically check for updates by retrieving data from a BOOSTDEVELOPERS server. If a newer version is found an admin notice will appear. You can uncheck this option if you do not want to check for updates or if you simply want the admin notice to go away.
						</span>	
						<br><br>
					</div>
					...
					<table class='form-table'>
						...
						<tr valign='top'>
							<th scope='row'>Automatically check for updates</th>
							<td>
								<?php 
									if (ini_get('allow_url_fopen') == 1) { ?>
										<input class="checkbox" type="checkbox" id="check_for_updates" name="inspire_options[check_for_updates]" value="checked" <?php checked(isset($inspire_options['check_for_updates'])) ?>/> 
									<?php } else {
										echo "<i>Your server configuration does not support the Inspire theme automatic update check. <br>
										You can go <a href='http://themeforest.net/item/inspire-a-multipurpose-masonry-theme/4023580' target='_blank'>here</a> to manually check for updates.</i>";	
									}
								?>
							</td>
						</tr>
