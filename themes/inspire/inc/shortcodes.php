...
****************************/
	...
	add_action('init', 'inspire_sc_addbuttons');
	function inspire_sc_addbuttons() {
	   if ( ! current_user_can('edit_posts') && ! current_user_can('edit_pages') )
	     return;
	 
	   // Add only in Rich Editor mode
	   if ( get_user_option('rich_editing') == 'true') {
	     add_filter("mce_external_plugins", "inspire_sc_add_tinymce_plugin");
	     add_filter('mce_buttons_3', 'inspire_sc_register_button');
	   }
	}

	function inspire_sc_register_button($buttons) {
	   array_push($buttons,"shortcodeselect"); //"seperator" will make a short space between buttons
	   return $buttons;
	}
	 
	function inspire_sc_add_tinymce_plugin($plugin_array) {
