    ...
    */
    function render() {
        ...
        if(get_bloginfo('version') >= '3.5') {
            echo '<input type="text" id="' . $this->field['id'] . '" name="' . $name . '" value="' . $this->value . '" class="' . $class . ' popup-colorpicker" style="width: 70px;" data-default-color="' . ( isset($this->field['std']) ? esc_attr($this->field['std']) : '' ) . '"/>';
            echo (isset($this->field['desc']) && !empty($this->field['desc'])) ? ' <span class="description">' . $this->field['desc'] . '</span>' : '';
        } else {
            echo '<div class="farb-popup-wrapper">';
            echo '<input type="text" id="' . $this->field['id'] . '" name="' . $name . '" value="' . $this->value . '" class="' . $class . ' popup-colorpicker" style="width:70px;"/>';
