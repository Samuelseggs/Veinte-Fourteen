...
class Dynamic_Styles {
	...
	public function set($element, $attr, $option_key, $important = false) {
		...
		if($this->use_options)
			$value = array($attr => $this->get_option($option_key));
		else
			$value = $option_key;	
	...
	public function setBackground($element, $option_key, $important = false) {
		...
		if($this->use_options) {
			...
			$styles[$element] = array(
				'background-image' => $this->get_option($option_key, 'file'),
				'background-repeat' => $this->get_option($option_key, 'repeat'),
				'background-size' => $this->get_option($option_key, 'size'),
				'background-position' => $this->get_option($option_key, 'position'),
				'background-attachment' => $this->get_option($option_key, 'attachment'),
				'background-color' => $this->get_option($option_key, 'color')
			);
	...
	public function setFont($element, $option_key, $important = false) {
		...
		if($this->use_options) {
			...
			$fonts = array();
			if(!empty($font) && strpos($font, "_safe_") === false) {
	...
	public function render() {
		...
		$css .= $this->get_option('custom_css');
	...
	protected function get_option($id, $key = null) {
