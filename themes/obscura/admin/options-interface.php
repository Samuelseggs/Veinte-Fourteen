...
function optionsframework_fields() {
	...
	foreach ($options as $value) {
		...
		case 'size':	
			...
			for ($i = 9; $i < 71; $i++) { 
				$size = $i . 'px';
				$output .= '<option value="' . esc_attr( $size ) . '" ' . selected( $size_stored['size'], $size, false ) . '>' . esc_html( $size ) . '</option>';
			}
			$output .= '</select>';
			
			// Font Size
			$output .= '<select class="of-typography of-typography-size" name="' . esc_attr( $option_name . '[' . $value['id'] . '][line]' ) . '" id="' . esc_attr( $value['id'] . '_line' ) . '">';
			for ($i = 0; $i < 71; $i++) { 
				$line = $i . 'px';
				$output .= '<option value="' . esc_attr( $line ) . '" ' . selected( $size_stored['line'], $line, false ) . '>' . esc_html( $line ) . '</option>';
			}
			$output .= '</select>';
