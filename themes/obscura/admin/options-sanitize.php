...
function of_sanitize_size( $input ) {
	$output = wp_parse_args( $input, array(
		'size'  => ''
	) );
...
add_filter( 'of_sanitize_size', 'of_sanitize_size' );

function of_sanitize_line( $input ) {
	$output = wp_parse_args( $input, array(
		'line'  => ''
	) );

	$output['line']  = apply_filters( 'of_font_line', $output['line'] );

	return $output;
}
add_filter( 'of_sanitize_line', 'of_sanitize_line' );
...
function of_sanitize_font_line( $value ) {
	$recognized = of_recognized_font_lines();
	$value = preg_replace('/px/','', $value);
	if ( in_array( (int) $value, $recognized ) ) {
		return (int) $value;
	}
	return (int) apply_filters( 'of_default_font_line', $recognized );
}
add_filter( 'of_font_face', 'of_sanitize_font_face' );
