...
global $THEMEREX_cat_options, $THEMEREX_post_options, $THEMEREX_custom_options, $wp_query;
// Current post & category custom options
$THEMEREX_post_options = $THEMEREX_cat_options = array();
if (is_category()) {
	...
	$THEMEREX_post_options = get_post_meta($page_id, 'post_custom_options', true);
} else if (is_single() || is_page() || $wp_query->is_posts_page==1) {
	// Current post custom options
	$page_id = is_single() || is_page() ? get_the_ID() : $wp_query->queried_object_id;	//getTemplatePageId('template-blog');
	$THEMEREX_post_options = get_post_meta($page_id, 'post_custom_options', true);
...
<body <?php 
	...
	body_class('theme_'.get_custom_option('blog_theme') . ' ' . (get_theme_option('puzzles_animations')=='yes' ? 'puzzles_animations ' : '') . $body_style . ($class!='' ? ' ' . $class : ''));
