...
function my_strrpos($text, $char, $from=0) { return MULTIBYTE ? mb_strrpos($text, $char, $from) : strrpos($text, $char, $from); }
...
function my_strtoproper($text) { 
	$rez = ''; $last = ' ';
	for ($i=0; $i<my_strlen($text); $i++) {
		$ch = my_substr($text, $i, 1);
		$rez .= my_strpos(' .,:;?!()[]{}+=', $last)!==false ? my_strtoupper($ch) : my_strtolower($ch);
		$last = $ch;
	}
	return $rez;
}
...
function dateDifference($dt1, $dt2=null, $short=true, $sec = false) {
	...
	$rez = '';
	if ($days > 0)
		$rez .= ($rez!='' ? ' ' : '') . $days . ' ' . __('day', 'themerex') . ($days > 1 ? 's' : '');
	if ((!$short || $rez=='') && $hours > 0)
		$rez .= ($rez!='' ? ' ' : '') . $hours . ' ' . __('hour', 'themerex') . ($hours > 1 ? 's' : '');
	if ((!$short || $rez=='') && $min > 0)
		$rez .=  ($rez!='' ? ' ' : '') . $min . ' ' . __('minute', 'themerex') . ($min > 1 ? 's' : '');
	if ($sec || $rez=='')
		$rez .=  $rez!='' || $sec ? (' ' . $diff . ' ' . __('second', 'themerex') . ($diff > 1 ? 's' : '')) : __('less then minute'. 'themerex');
	return $rez;
}
...
function getVideoPlayerURL($url, $autoplay=false) {
	$url = str_replace(
		...
		trim(chop($url)));
	if ($autoplay && $url!='') {
		if (my_strpos($url, 'autoplay')===false) {
			$url .= (my_strpos($url, '?')===false ? '?' : '&') . 'autoplay=1';
		}
	}
	return $url;
}
...
function getSelectorPropertyFromCSS($css, $selector, $prop) {
	...
	$file = file($css);
	foreach ($file as $row) {
		if (($pos = my_strpos($row, $selector))===false) continue;
		if (($pos2 = my_strpos($row, $prop.':', $pos))!==false && ($pos3 = my_strpos($row, ';', $pos2))!==false && $pos2 < $pos3) {
			$rez = trim(chop(my_substr($row, $pos2+my_strlen($prop)+1, $pos3-$pos2-my_strlen($prop)-1)));
			break;
		}
	}
	return $rez;
