...
function add_sc_select(){
	...
	$shortcodes_list .= '<option value="'
		. "[blogger ids='' cat='' orderby='date' order='desc' count='3' descr='200' style='bubble_left' border='0' dir='horizontal']"
	...
	$shortcodes_list .= '<option value="'
		."[googlemap address='' latlng='' zoom='16' width='100%' height='240']"
	...
	$shortcodes_list .= '<option value="'
		. "[title type='1' style='bubble_left' bubble_color='#00bcff' icon='spin2']".__('Title text here', 'themerex')."[/title]"
...
function sc_audio($atts, $content = null) {
	...
	$s = ($top !== '' ? 'margin-top:' . $top . 'px;' : '')
		...
		.($right !== '' ? 'margin-right:' . $right . 'px;' : '')
		;
	return '<audio' . ($id ? ' id="' . $id . '"' : '') . ' src="' . ($src!='' ? $src : $url) . '" class="sc_audio" ' . ($controls ? ' controls="controls"' : '') . ' width="' . $width . '" height="' . $height .'"'.($s!='' ? ' style="'.$s.'"' : '').'></audio>';
}
...
function sc_blogger($atts, $content=null){	
	...
	if (!in_array($style, array('regular','date','image_large','image_medium','image_small','bubble_left','bubble_top','accordion','puzzles')))
		$style='regular';	
	if (!empty($ids)) {
	...
	while ( $query->have_posts() ) { 
		...
		$post_date_diff = getDateOrDifference(get_the_date('Y-m-d H:i:s'));
...
/*
		$post_excerpt = $post_format=='quote' 
			? $post_content_prepared
			: getShortString(trim(chop($post->post_excerpt))!='' ? trim(chop(do_shortcode($post->post_excerpt))) : strip_tags($post_content_prepared), $descr, $readmore ? '' : '...');
*/
		$post_excerpt = $post_format=='quote' 
			? $post_content_prepared 
			: strip_tags(strip_shortcodes(getPostDescription($descr, $readmore ? '' : '...')));
		...
		if (in_array($style, array('image_medium', 'image_large', 'puzzles'))) {
			if ($post_format == 'gallery') {
				...
				$post_url = getPostLink($post_content_prepared, false);
			}
		}
		...
		if ($style == 'puzzles' ) {
			...
			$output .= '<div class="post_thumb image_wrapper post_format_'.$post_format.' '.(in_array($post_format, array('quote', 'link', 'image')) || (!$post_thumb && (!$post_gallery || $post_protected)) ? 'no_thumb' : $post_hover_pos) .'"' . ($post_video && !$post_protected ? ' data-video="'.htmlspecialchars($post_video).'"' : '').'>';
			if ($post_gallery && !$post_protected) {		// If post have gallery - show it
				...
				if ($post_format=='link' && $post_url!='')
					$output .= '<a href="'.$post_url.'">'.$post_thumb.'</a>';
				else if ($post_link!='')
			...
			$output .= '<span class="post_format theme_accent_bg '.$post_icon.'"'.(my_substr($post_accent_color, 0, 1)=='#' ? ' style="background-color: '.$post_accent_color.'"' : '').'></span>';
			if ($post_accent_category!='') { 
				$output .= '<span class="post_category theme_accent_bg"'.(my_substr($post_accent_color, 0, 1)=='#' ? ' style="background-color: '.$post_accent_color.'"' : '').'>'.$post_accent_category.'</span>';
			}
			if ($show_content_block) {
				...
				if (!in_array($post_format, array('quote', 'aside'))) { 
					...
					if ( get_custom_option('show_reviews', null, $post_id)=='yes' ) {
						...
						if ($avg_author > 0) {
							$output .= '<div class="reviews_summary blog_reviews"><div class="criteria_summary criteria_row">' . getReviewsSummaryStars($avg_author) . '</div></div>';
						}
					} 
				}
			...
			if ($style == 'date' ) {
				...
				$output .= ($style == 'image_small' ? '' : '<div class="sc_blogger_image image_wrapper">' . $post_thumb . '</div>') 
			...
			if (my_strpos($style, 'image')!==false ) {
				$output .= '</div>' . ($style == 'image_small' ? '<div class="sc_blogger_image image_wrapper">' . $post_thumb . '</div>' : '');
			}
			if ($descr > 0) {
...
function sc_button($atts, $content=null){	
	...
	return empty($link) ? '' : '<a href="' . $link . '"' . (!empty($target) ? ' target="' . $target . '"' : '') . ($id ? ' id="' . $id . '"' : '') . ' class="sc_button sc_button_style_' . $style . ($style=='regular' ? ' theme_button' : '') . ' sc_button_size_' . $size . ($align ? ' align'.$align : '') . '"'.($s!='' ? ' style="'.$s.'"' : '').'>' . do_shortcode($content) . '</a>';
}
...
function sc_contact_form($atts, $content = null) {
	...
	$s = ($top !== '' ? 'margin-top:' . $top . 'px;' : '')
		...
		.($right !== '' ? 'margin-right:' . $right . 'px;' : '')
		;
	wp_enqueue_script( 'contact_form', get_template_directory_uri().'/js/_contact_form.js', array('jquery'), '1.0.0', true );
...
function sc_google_map($atts, $content = null) {
	...
	wp_enqueue_script( 'googlemap_init', get_template_directory_uri().'/js/_googlemap_init.js', array(), '1.0.0', true );
		...
		while ( $query->have_posts() ) { 
			...
			if ($titles) {
				...
				if (get_custom_option("slider_info_category")=='yes') { // || empty($cat)) {
					...
					if ($post_accent_category!='') { 
						$output .= '<div class="sc_slider_category theme_accent_bg"'.(my_substr($post_accent_color, 0, 1)=='#' ? ' style="background-color: '.$post_accent_color.'"' : '').'>'.$post_accent_category.'</div>';
					}
				}
				$output_reviews = '';
				if (get_custom_option('show_reviews')=='yes' && get_custom_option('slider_reviews')=='yes') {
					$avg_author = get_post_meta($post_id, 'reviews_avg'.((get_theme_option('reviews_first')=='author' && $orderby != 'users_rating') || $orderby == 'author_rating' ? '' : '2'), true);
					if ($avg_author > 0) {
...
function sc_title($atts, $content=null){	
	...
	$s = ($top !== '' ? 'margin-top:' . $top . 'px;' : '')
		...
		.($left !== '' ? 'margin-left:' . $left . 'px;' : '')
		.($right !== '' ? 'margin-right:' . $right . 'px;' : '')
		//.(my_substr($style, 0, 4)=='icon' ? 'background-image:url('.get_template_directory_uri().'/images/icons/'.$icon.'.png);' : '')
	...
	return '<h' . $type . ($id ? ' id="' . $id . '"' : '')
		. ($style=='underline' 
			? ' class="sc_title"' 
			: (my_strpos($style, 'bubble')!==false ? ' class="sc_title_bubble sc_title_'.$style.'"' : (my_strpos($style, 'icon')!==false ? ' class="sc_title_icon sc_title_'.$style.'"' : '')))
		. ($s!='' ? ' style="'.$s.'"' : '')
		. '>'
		. (my_substr($style, 0, 6)=='bubble' 
			...
			: (my_substr($style, 0, 4)=='icon' ? '<img src="'.get_template_directory_uri().'/images/icons/'.$icon.'.png" />'.($style=='icon_top' ? '<br />' : '') : ''))
		. do_shortcode($content) 
		. '</h' . $type . '>';
}
...
function sc_video($atts, $content = null) {
	...
	return '<video' . ($id ? ' id="' . $id . '"' : '') . ' class="sc_video" src="' . $url . '" width="' . $width . '" height="' . $height . '"'.($s!='' ? ' style="'.$s.'"' : '').' controls="controls"></video>';
/*
