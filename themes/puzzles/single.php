				...
				<article <?php post_class('theme_article post_format_'.$post_format); ?>>
					...
					<div class="post_content<?php echo get_custom_option('without_paddings')=='yes' ? ' without_paddings' : ''; ?>">
						...
						if ( get_custom_option('show_reviews')=='yes') {
							$avg_author = get_post_meta($post_id, 'reviews_avg', true);
							$avg_users  = get_post_meta($post_id, 'reviews_avg2', true);
							if ($avg_author > 0 || $avg_users > 0) {
								...
								if ($use_tabs) wp_enqueue_script('jquery-ui-tabs');
								?>	
								<div class="post_reviews theme_<?php echo get_custom_option('sidebar_main_theme'); ?>">
									...
									if ($reviews_first_author || !$reviews_second_hide) {
										...
										$marks = get_custom_option('reviews_marks');
										$output .= getReviewsMarkup($field, $marks, false);
										$output .= '</div>';
									}
									if (!$reviews_first_author || !$reviews_second_hide) {
										$output .= '<div id="reviews_users" class="reviews_tab reviews_users"'.(!$output ? ' style="display: block;"' : '').'>';
										$marks = get_post_meta($post_id, 'reviews_marks2', true);
										...
										$field["descr"] = '<div class="criteria_summary_descr">' 
											...
											. ' ' 
											.(!isset($_COOKIE['reviews_vote']) || my_strpos($_COOKIE['reviews_vote'], ','.$post_id.',')===false
								...
								<script type="text/javascript">
									...
									var allowUserReviews = <?php echo (!$reviews_first_author || !$reviews_second_hide) && (!isset($_COOKIE['reviews_vote']) || my_strpos($_COOKIE['reviews_vote'], ','.$post_id.',')===false) ? 'true' : 'false'; ?>;
					...
					?>
						<div class="post_author_details theme_article">
							...
							<div class="post_author_info">
								<div class="post_author_avatar image_wrapper"><a href="<?php echo $post_author_url; ?>"><?php echo $post_author_avatar; ?></a></div>
								<h5 class="post_author_name"><a href="<?php echo $post_author_url; ?>" class="theme_strong"><?php echo $post_author; ?></a></h5>
								<div class="post_author_description"><?php echo $post_author_descr; ?></div>
						...
						<div id="related_posts" class="theme_article">
							...
							foreach( $recent_posts as $recent ) {
								...
								if ($recent['post_format'] == 'gallery') {
									...
									if (get_custom_option('substitute_video')=='yes') {
										$src = getVideoPlayerURL(getPostVideo($recent['post_video']), true);
									...
									$recent['post_url'] = getPostLink($recent['post_content_prepared'], false);
								}
								...
								<div class="related_posts_item related_post_item_<?php echo $i; ?> post_format_<?php echo $recent['post_format']; ?> <?php echo ($i % 2 == 0 ? 'even' : 'odd') . ($i==1 ? ' first' : '') . ($i==3+$thumb_idx ? ' last' : ''); ?> post_thumb image_wrapper <?php echo $recent['post_format']=='quote' || (!$recent['post_thumb'] && (!$recent['post_gallery'] || $recent['post_protected'])) ? 'no_thumb' : $recent['post_hover_pos']; ?>"<?php echo $recent['post_video'] && !$recent['post_protected'] ? ' data-video="'.htmlspecialchars($recent['post_video']).'"' : ''; ?>>
									...
									if ($recent['post_gallery'] && !$recent['post_protected']) {		// If post have gallery - show it
										...
										if ($recent['post_format']=='link' && $recent['post_url']!='')
											echo '<a href="'.$recent['post_url'].'">'.$recent['post_thumb'].'</a>';
									...
									?>
									<span class="post_format theme_accent_bg <?php echo $recent['post_icon']; ?>"<?php echo my_substr($recent['post_accent_color'], 0, 1)=='#' ? ' style="background-color: '.$recent['post_accent_color'].'"' : ''; ?>></span>
									<?php if (count($recent['categories']) > 0) { ?>
										<span class="post_category theme_accent_bg"<?php echo my_substr($recent['post_accent_color'], 0, 1)=='#' ? ' style="background-color: '.$recent['post_accent_color'].'"' : ''; ?>><?php echo $recent['categories'][0]['name']; ?></span>
									<?php 
									} 
									...
									<div class="post_content_wrapper theme_puzzles"<?php echo $recent['post_hover_bg']!='' && $recent['post_hover_bg']!='default' ? ' style="background-color:'.$recent['post_hover_bg'].';"' : ''; ?>>
										<?php if (!in_array($recent['post_format'], array('quote', 'aside'))) { ?>
											...
											if ( get_custom_option('show_reviews', null, $recent['ID'])=='yes' ) {
												$avg_author = get_post_meta($recent['ID'], 'reviews_avg', true);
												if ($avg_author > 0) {
													?>
