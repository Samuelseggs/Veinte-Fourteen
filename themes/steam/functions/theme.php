...
function it_pagination($pages = '', $format, $range = 6) {	
	...
	if(1 != $pages)	{
		...
		for ($i = $start; $i <= $stop; $i++) {	
			if($i>0 && $i<=$pages) {
				...
				if($start > 1 && !$leftshown && !it_get_setting('prev_next_disable')) {
					$leftshown = true;
					$leftnum = $i - 1;
					$out .= '<a class="inactive arrow previous" data-paginated="' . $leftnum . '"><span class="icon-previous"></span></a>';	
				}
				#page number
		...
		if($stop < $pages) {
			$rightnum = $i + 1;
			if(!it_get_setting('prev_next_disable')) $out .= '<a class="inactive arrow next" data-paginated="' . $rightnum . '"><span class="icon-next"></span></a>';	
...
function it_get_postnav() {
	...
	$out .= '<div id="postnav-wrapper" class="clearfix">';	
		$out .= '<div id="postnav">';		
...
function it_section_menu() {
	...
	if ( ( $locations = get_nav_menu_locations() ) && isset( $locations[ 'main-menu' ] ) ) {
		...
		foreach ( (array) $menu_items as $key => $menu_item ) {
			...
			$out .= '<li id="menu-item-' . $objectid . '" class="menu-item unloaded menu-item-' . $objectid . ' ' . $type . ' ' . $minisiteid . ' ' . $current . '" data-loop="menu" data-method="' . $method . '" data-minisite="' . $minisiteid . '" data-numarticles="' . $numarticles . '" data-object_name="'.$object_name.'" data-object="'.$object.'" data-thumbnail="'.$thumbnail.'" data-type="'.$menu_item_type.'" data-sorter="'.$term_slug.'">';
				$out .= '<a class="parent-item' . $arrow_right . '" href="' . $url . '">' . $icon . $title . $arrow . '</a>';
				$out .= $loading;
				$out .= '<div class="dropdown-placeholder"></div>';
			$out .= '</li>';
		}		
	}
	$out .= '</ul>';	
...
function it_featured_image($postid, $size, $width, $height, $wrapper = false, $itemprop = false, $link = true) {
    ...
    if($link) $out .= '<a class="darken featured-image" href="' . $large_image_url[0] . '">';
	$out .= $featured_image;
	if($link) $out .= '</a>';
...
function it_get_likes($args) {
	...
	if($clickable) {
		$out.='<a class="like-button '.$postid.$tooltip.'" data-postid="'.$postid.'" data-likeaction="'.$likeaction.'" title="'.__('Likes',IT_TEXTDOMAIN).'">';
	} else {
...
function it_login_form() {
	...
	$out .= '<form method="post" action="' . home_url() . '/wp-login.php" class="sticky-login-form">';
...
function it_register_form() {
	...
	$out .= '<form method="post" action="' . site_url('wp-login.php?action=register', 'login_post') . '" class="sticky-register-form">';
		...
		foreach ( $social_keys as $key ) {
			if( $key != '#' ) {
				...
				if( !empty( $social[$key]['custom'] ) )
					$out .= '<a href="'.esc_url( $social_link ).'" class="info" title="'.$social_hover.'"><img src="' . esc_url( $social_custom ) . '" alt="link" /></a>';

				elseif( empty( $social[$key]['custom'] ) )
					$out .= '<a href="'.esc_url( $social_link ).'" class="icon-'.$social_icon.' info" title="'.$social_hover.'"></a>';
		...
		case 'categories':
			...
			foreach($terms as $term) {				
				...
				if(have_posts()) {
					...
					if($count==1) $filters .= ' active';
					$filters .= '" title="' . __('More articles in category: ', IT_TEXTDOMAIN) . $term->name . '">'.$term->name.'<span class="bottom-arrow"></span></a>';				
